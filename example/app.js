...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      tabs: [
        ...
        <Tab
          key={'tab5'}
          title={'Big content1 with left:-9999999px'}
          hiddenContainerStyle={{ left: '-9999999px', top: '-9999999px' }}
          {...this.makeListeners('tab5')}
        >
          <div>
  ...
  makeListeners(key) {
    return {
      onClick: e => {
        console.log('onClick', key, e);
      }, // never called
      onContextMenu: e => {
        console.log('onContextMenu', key, e);
  ...
  closeFromContextMenu() {
    let newTabs = this.state.tabs.filter(t => {
  ...
  _onDialogShow() {
    let tab = this.state.tabs.find(t => {
  ...
  render() {
    let standardActions = [
      <FlatButton label="Cancel" secondary={true} onClick={this._onDialogCancel.bind(this)} />,
      <FlatButton label="Submit" primary={true} keyboardFocused={true} onClick={this._onDialogSubmit.bind(this)} />,
    ];
    ...
    return (
      <div>
        ...
        <Dialog title="Change tab name" ref="dialog" actions={standardActions} modal={true} open={this.state.dialogOpen} onShow={this._onDialogShow.bind(this)}>
          <TextField ref="input" id="rename-input" style={{ width: '90%' }} />
        </Dialog>
